...
시스템은 PCG 파라미터를 생성하지만, **실제 맵 생성은 사용자의 PCG 도구**(예: TileWorldCreator)에서 이루어집니다. 도구 설정이 사용자마다 다르기 때문에 시각적 피드백 루프는 수동으로 진행됩니다:
...
> **도구 설정이 사용자마다 다르기 때문입니다.** 각 사용자는 고유한 PCG 도구 설정, 에셋 라이브러리, Unity 프로젝트 구조를 가지고 있습니다. 시스템이 도구를 자동 실행할 수 없는 이유:
> - TileWorldCreator API가 버전마다 다름
> - 커스텀 도구 설정이 사용자마다 다름
> - 에셋 경로가 사용자별로 다름
> - Unity 프로젝트 구조가 다름
>
> Dual-Agent 시스템은 **파라미터 생성**에 집중하고, 실행은 사용자 환경에 맡깁니다.

#### 예시 워크플로우

```bash
# 1. 레퍼런스 이미지로 초기 파라미터 생성
/Map ~/reference/mountain_village.png

# 2. 생성된 JSON을 TileWorldCreator에 적용 (수동)
#    → 맵 생성 → 결과 스크린샷

# 3. 결과 기반 개선
/Map ~/screenshots/attempt1.png "산의 높이 변화가 더 필요함"

# 4. 개선된 파라미터 적용 (수동)
#    → 맵 생성 → 결과 스크린샷

# 5. 만족할 때까지 계속
/Map ~/screenshots/attempt2.png "경사면에 나무를 더 추가"
```
